...
layer1 = tf.layers.Dense(
            ...
            use_bias = True,
            kernel_initializer = tf.random_uniform_initializer(minval = -0.5, maxval = 0.5, seed = seed_value),
            bias_initializer = tf.random_uniform_initializer(minval = -0.5, maxval = 0.5, seed = seed_value),
...
with tf.name_scope("ranik"):
    ...
    with tf.name_scope("update"):
        op1=tf.assign(lay_1_kernel, lay_1_kernel-((epsilon + (loss * gra_lay_1_kernel)) / (loss + tf.square(gra_lay_1_kernel))))
        op2=tf.assign(lay_1_biases, lay_1_biases-((epsilon + (loss * gra_lay_1_biases)) / (loss + tf.square(gra_lay_1_biases))))
        op3=tf.assign(lay_2_kernel, lay_2_kernel-((epsilon + (loss * gra_lay_2_kernel)) / (loss + tf.square(gra_lay_2_kernel))))
        op4=tf.assign(lay_2_biases, lay_2_biases-((epsilon + (loss * gra_lay_2_biases)) / (loss + tf.square(gra_lay_2_biases))))
        op5=tf.assign(lay_3_kernel, lay_3_kernel-((epsilon + (loss * gra_lay_3_kernel)) / (loss + tf.square(gra_lay_3_kernel))))
        op6=tf.assign(lay_3_biases, lay_3_biases-((epsilon + (loss * gra_lay_3_biases)) / (loss + tf.square(gra_lay_3_biases))))
...
model_name = "RanikOptimizer_random_uniform_initialization_and_sigmoid_activation_Model"
model_save_path = os.path.join(base_model_path, model_name)
